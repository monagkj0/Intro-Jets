...
int traceToHardProcess(const Particle& particle, const Event& event) {
    ...
    while (current > 0) {
        ...
        if (mother.statusAbs() == 23) { 
            // std::cout << mother.statusAbs() << std::endl;
            return mother.id();
        }
...
int main(int argc, char* argv[]) {
    ...
    // Set up ROOT output
    std::string filename = "dijet_events_" + std::to_string(seed) + ".root";
    ...
    // Declare jet constituent momentum vectors (as TVector3)
    std::vector<TVector3> jet1_Cons_Pxyz;
    std::vector<TVector3> jet2_Cons_Pxyz;
    ...
    tree->Branch("jet1_Cons_Pxyz",&jet1_Cons_Pxyz);
    ...
    tree->Branch("jet2_Cons_Pxyz",&jet2_Cons_Pxyz);
    ...
    for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
        ...
        jet1_Cons_Pxyz.clear();
        jet2_Cons_Pxyz.clear();
        jet1_Cons_ID.clear();
        jet2_Cons_ID.clear();
        jet1_Cons_E.clear();
        jet2_Cons_E.clear();        
        ...
        for (const PseudoJet& c : jet1.constituents()) {
            jet1_Cons_Pxyz.emplace_back(c.px(), c.py(), c.pz());
        ...
        for (const PseudoJet& c : jet2.constituents()) {
            jet2_Cons_Pxyz.emplace_back(c.px(), c.py(), c.pz());
