...
int main(int argc, char* argv[]) {
    ...
    if (argc > 2) etaLL = std::stoi(argv[2]);
    if (argc > 3) etaRR = std::stoi(argv[3]);
    if (argc > 4) etaRL = std::stoi(argv[5]);
    ...
    pythia.readString("ContactInteractions:Lambda = 10000"); //+ std::to_string(lambdaT)); 
    pythia.readString("ContactInteractions:QCqq2qq = on");
    pythia.readString("ContactInteractions:QCqqbar2qqbar = on");

    pythia.readString("ContactInteractions:etaLL = " + std::to_string(etaLL)); 
    pythia.readString("ContactInteractions:etaRR = " + std::to_string(etaRR)); 
    pythia.readString("ContactInteractions:etaRL = " + std::to_string(etaRL));     
    ...
    // Set up ROOT output
    std::string filename = "dijet_events_CI_" + std::to_string(seed) + "_" + std::to_string(etaLL) + std::to_string(etaRR) + std::to_string(etaRL) + ".root";
